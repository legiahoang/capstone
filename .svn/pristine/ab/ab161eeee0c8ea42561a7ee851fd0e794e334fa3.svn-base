...
function scrollMenu() {
	if (document.body.scrollTop > 10 || document.documentElement.scrollTop > 10) {
		...
		document.getElementById("header_form:header_menu").classList
				.remove("fix");
	}
}
...
function encrypt() {
	...
	var encypted = MD5(password);
	$('#loginForm\\:hdpassword').val(encypted);
}
...
function answerComment(textArea) {
	var value = $(textArea).val();
	$('#auction_form\\:hd_content_answer_comment').val(value);
	$(textArea).val('')
}

// Show reply comment box
var isReplyCommentBoxHidden = true;
function showReplyCommentBox(relyCommentBoxId) {
	$('#reply_comment_box' + relyCommentBoxId).css('display', '');
}
