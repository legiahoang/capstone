package vn.co.cex.bean.goodsowner;
...
@ManagedBean(name = "goodsOwnerBillOfLadingBean", eager = true)
@ViewScoped
public class GoodsOwnerBillOfLadingBean extends BaseBean {

	private static final Logger log = LogManager.getLogger(GoodsOwnerBillOfLadingBean.class);
	...
	public void init() {
		try {
			...
			searchBillOfLadingOfGoodsOwner();
		} catch (Exception e) {
			log.debug(e);
		}
	}
	 ...
	 */
	public void searchBillOfLadingOfGoodsOwner() {
		try {
			billOfLadings = new LazyDataModel<BillOfLadingDTO>() {
				...
				public List<BillOfLadingDTO> load(int first, int pageSize, String sortField, SortOrder sortOrder,
					...
					List<BillOfLadingDTO> list = billOfLadingBO.searchBillOfLadingOfGoodsOwner(goodsTypeId,
							packagedFormId, weight, volume, departureProvinceCode, arrivalProvinceCode, departureDate,
							arrivalDate, user.getId(), pageSize, first);
					return list;
				}
			};
			// Count total items
			int count = billOfLadingBO.countBillOfLadingOfGoodsOwner(goodsTypeId, packagedFormId, weight, volume,
